...
const MACRO_GROUPS: { label: string; symbols: string[] }[] = [
  { label: '달러 / 금리 / 채권', symbols: ['DX-Y.NYB', '^TNX', 'TLT'] },
  { label: '원자재', symbols: ['CL=F', 'GLD'] },
  { label: '지수', symbols: ['SPY', 'QQQ'] },
  ...
  return (
    <div className="glass-card rounded-xl p-3.5 flex flex-col gap-2.5 hover:bg-zinc-900/30 transition-all duration-200 relative overflow-hidden">
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="font-bold text-sm text-zinc-100 tracking-tight">{item.symbol}</div>
          <div className="text-[10px] text-zinc-500 font-medium mt-0.5 leading-tight">{item.name}</div>
        </div>
      ...
      {/* EMA status + RSI */}
      <div className="flex items-center justify-between border-t border-zinc-800/60 pt-2 gap-2">
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-32 space-y-4">
        ...
        <div className="text-zinc-500 text-sm tracking-wide">매크로 지표 불러오는 중...</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="glass-card rounded-2xl p-5 shadow-2xl relative overflow-hidden">
        ...
        <div className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest flex items-center gap-1.5 mb-4">
          <span className="w-1.5 h-1.5 rounded-full bg-zinc-600" />
          섹터 로테이션 — 1일 수익률 기준 (오늘 상/하위)
        </div>
        <div className="flex flex-col gap-2">
          {sortedSectors.map((item) => {
            const maxAbs = Math.max(...sortedSectors.map((s) => Math.abs(s.change_pct_1d)), 0.5);
            const barW = Math.abs(item.change_pct_1d) / maxAbs * 60;
            const pos = item.change_pct_1d >= 0;
            return (
              <div key={item.symbol} className="flex items-center gap-3">
                ...
                <div className="flex-1 flex items-center h-5">
                  {pos ? (
                    <div
                      className="h-4 rounded-r-full bg-gradient-to-r from-emerald-500/60 to-emerald-400/80 transition-all duration-500"
                      style={{ width: `${barW}%` }}
                    />
                  ) : (
                    <div
                      className="h-4 rounded-r-full bg-gradient-to-r from-red-500/60 to-red-400/80 transition-all duration-500 ml-auto"
                      style={{ width: `${barW}%` }}
                    />
                  )}
                </div>
      ...
      {MACRO_GROUPS.map((group) => {
        const items = group.symbols.map((s) => bySymbol[s]).filter(Boolean);
      ...
      <p className="text-[11px] text-zinc-600 font-medium text-center pb-2">
