...
## 8. Conviction 사용 가이드라인 (Phase 1 마무리 작업 완료)

**Phase 1 마무리 후 현재 상태**
- 신뢰도(reliability) + notes 시스템 도입
- 에러 발생 시에도 API가 안전하게 동작
- 로딩 상태 및 UI 가독성 개선
- 실제 사용 시 주의사항 문서화

**신뢰할 수 있는 경우 (High Confidence)**
- `reliability: "high"`
- `notes` 배열이 비어 있음
- 최근 데이터 기반 + Regime 정보 정상

**주의해야 하는 경우 (Medium / Low)**
- `reliability: "low"` 또는 "medium"
- notes에 경고 메시지가 있는 경우
- 큰 시장 이벤트 직후, 데이터 지연 시

**절대 금지**
- Conviction 점수만으로 즉시 매매 결정
- 신뢰도가 낮은 상태에서 큰 포지션 진입

**권장 사용법**
1. Conviction + 기존 Stage2/RS + 본인 분석을 종합
2. reliability가 high일 때만 적극 참고
3. low일 때는 "관망" 신호로만 활용

Conviction은 **의사결정 지원 도구**이며, Phase 1 기준으로는 아직 자동매매용으로는 부적합합니다.
...
**Phase 1 마무리 결론**
- 핵심 기능 (Conviction v1 + Context Attribution)은 모두 구현 완료
- 신뢰성, 에러 대응, 사용성, UI, 문서화까지 1차 마무리 작업 완료
- "실제 투자 판단에 참고할 수 있는 수준"까지 끌어올림

모든 주요 변경은 `feat/yf-accuracy-harden-2026-05-25` 브랜치에 커밋되어 있으며, CLAUDE.md 규칙을 준수했습니다.

Phase 1은 기능적으로 완료되었으며, 이제 더 보수적이고 신중하게 실제 환경에서 테스트할 수 있는 상태입니다.
