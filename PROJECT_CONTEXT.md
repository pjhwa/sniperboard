...
| Conviction Composite Score v1 (Phase 1) | `backend/core/conviction_calculator.py` (pure TDD calculator, 40/30/30, Stage2 'score' + regime 'total' + sentiment). `backend/tests/test_conviction_calculator.py` (5 tests). Exposed in /api/watchlist + /api/daily + minimal WatchlistBoard UI. Verification script: `scripts/verify_conviction.sh` (2026-05-25). |
| Phase 1 검증 스크립트 | `scripts/verify_conviction.sh` — watchlist/daily/brief의 conviction + context 필드 자동 확인 |
