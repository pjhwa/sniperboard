...
| Conviction Composite Score v1 (Phase 1) | `backend/core/conviction_calculator.py` (pure TDD calculator, 40/30/30, Stage2 'score' + regime 'total' + sentiment). `backend/tests/test_conviction_calculator.py`. Exposed in /api/watchlist via WatchlistItemSchema + get_watchlist_endpoint (2026-05-25). |
| Watchlist 항목에 신규 필드 추가 | `backend/api/schemas.py: WatchlistItemSchema` + `endpoints.py: get_watchlist_endpoint` |
