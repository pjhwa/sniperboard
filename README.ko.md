...
프론트엔드는 **상대 URL** (`/api/*`)을 사용하며, Next.js가 백엔드로 프록시합니다. IP가 번들에 포함되지 않으므로 VPN 환경에서도 정상 동작합니다.

| 변수 | 위치 | 기본값 | 설명 |
|------|------|--------|------|
| `BACKEND_URL` | `docker-compose.yml` 빌드 인자 + 환경 변수 | `http://localhost:5001` | Next.js API 프록시 rewrites에 사용되는 백엔드 URL. **빌드 시 평가** → 변경 시 재빌드 필요. |

> **Docker Compose**: `BACKEND_URL=http://backend:8000`이 빌드 인자와 런타임 환경 변수로 자동 주입됩니다.
> **로컬 개발 (Docker 없이)**: `npm run dev` 전에 `BACKEND_URL=http://localhost:8000`을 셸에서 설정하세요.
...
- **R:R 진입 계획**: 셋업 상태 배지(준비/관망/무효·회피) + Entry까지 거리% + 시스템 피벗 Entry/Stop/Target(20일 고점×1.005, 공식 불변) + 빨강1:녹색3 바 + 포지션(Ready일 때만 강조). Entry에서 멀리 떨어지거나 Stage2가 약하면 수치를 흐리게 하고 깊은 조정 안내. 보조 **「즉시 시장가 진입 시」** ATR 예시는 참고용(시스템 신호 아님).
...
- **R:R 패널**: 셋업 상태(준비/관망/무효) + Entry 거리 + 피벗 Entry/Stop/Target + 포지션(Ready일 때) + 즉시 진입 참고 수치. 카드 ⓘ 버튼으로 R:R 개념 팝오버 설명 제공.
...
- TIER1 (TSM · NVDA · META · TSLA · PLTR · MU · CRWD · AMZN · MSFT · AAPL · GOOGL · SPCX) + TIER2 (RKLB · CEG · VST · ALAB · OKLO · APP · ANET · NVO · QBTS · SOFI) Stage 2 점수 내림차순 정렬
...
### Backtest — 과거 신호 성과 검증

Stage2 신호를 2019년부터 현재까지 시뮬레이션. 과적합 방지 설계.

| 설계 원칙 | 구현 내용 |
|-----------|-----------|
| **look-ahead bias 없음** | T-1 신호 → T일 체결 (당일 즉시 체결 금지). TDD로 검증. |
| **생존 편향 공개** | 현재 워치리스트 기준 — 대시보드에 명시. |
| **표본 내/외 분리** | IS: ~2023(훈련) / OOS: 2024~(실제 검증). OOS > IS = 과적합 없음. |
| **몬테카를로 검증** | 10,000 부트스트랩 리샘플링 → 양수 기대값 확률 99.8% |
| **슬리피지 포함** | 0.05% 포함. 수수료 0% (미국 브로커 기준). |

**최적 설정** (RS≥70 + SPY>EMA200 필터): 145거래 · 승률 38.6% · 기대값 +0.460R · OOS +0.511R

**AMZN 구조적 한계**: 모든 파라미터 조합에서 승률 21% — Stage2 피벗 돌파가 AMZN의 박스권 가격 구조와 구조적으로 불일치. 종목별 테이블에 명시.

---

### Signal Tracker — 라이브 vs. 백테스트 검증

신호 도구 중 SniperBoard를 차별화하는 핵심 화면.

| 섹션 | 내용 |
|------|------|
| **모델 헬스 배너** | ON_TRACK / WATCH / UNDERPERFORMING — +0.460R 백테스트 기준값 대비. 신뢰도: LOW(<30거래) → MEDIUM → HIGH(80+). |
| **KPI 비교** | 라이브 승률/기대값/손익비 vs. 백테스트 값. 차이값 강조. |
| **누적 R 곡선** | 라이브 자산곡선 + 백테스트 기준선(회색 점선) 동일 차트 동일 R 스케일 오버레이. |
| **현재 파이프라인** | PENDING(진입 대기) + ACTIVE(보유 중) 신호. 진입가/손절가/목표가 + R:R 표시. |
| **레짐별 성과** | RISK_ON / MIXED / RISK_OFF별 기대값 — 신호가 언제 작동하는지 파악. |
| **신호 이력 테이블** | 모든 로깅된 신호와 결과. WIN / LOSS / TIMEOUT / CANCELLED 필터. |

자동 로깅: `/watchlist`가 갱신될 때마다 Stage2 ≥ 5 신호가 자동 기록됩니다. 수동 입력 불필요.
...
매수 수량 = (계좌 × 리스크%) ÷ (진입가 − 손절가)   # UI는 Setup Ready일 때만 수량 강조
```

**셋업 상태** (`frontend/lib/entryPlan.ts`, DeepDive·Daily 공유):

| 상태 | 의미 |
|------|------|
| **셋업 준비** | Stage2 ≥ 5 이고 Entry ~5% 이내(또는 상회) — 돌파 시 계획 유효 |
| **관망** | 중간 거리 또는 Stage2 부분 충족 — 대기 |
| **셋업 무효 · 회피** | Entry 대비 멀리(>15%) 또는 Stage2 약함 — 매수 신호 아님 |

**Entry까지 거리** = `(진입가 − 현재가) / 현재가 × 100`.  
**즉시 시장가 진입** (보조) = 현재가 + 2×ATR 손절 + 1:3 목표 — 참고용, 시스템 피벗 계획을 대체하지 않음.
...
| `GET /backtest/result` | 캐시된 백테스트 결과 JSON (없으면 404) |
| `POST /backtest/run` | 백테스트 즉시 실행. 파라미터: `rs_threshold`, `use_spy_filter`, `threshold` |
| `POST /backtest/sweep` | 8개 파라미터 조합 일괄 실험 |
| `GET /signal-log` | 라이브 신호 로그 (자동 기록). 파라미터: `symbol`, `limit` |
| `GET /signal-log/stats` | 라이브 성과 통계 vs. 백테스트 기준값. 모델 헬스 + 레짐별 분해 포함. |
| `POST /signal-log/refresh` | PENDING/ACTIVE 신호 결과를 최신 가격으로 갱신 (백그라운드) |
