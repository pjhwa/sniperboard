...
> - `/api/watchlist` (per-symbol sentiment 우선 사용) 및 `/api/daily` 응답에 `conviction_score` + `conviction_label` 필드 추가 (Stage2 + Sentiment + Regime 기반 40/30/30 계산). 간단한 통합 테스트 추가됨.
